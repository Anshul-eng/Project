...
void bill(int y, int j)                     //j=no. of passengers,y=bill amount
{
    ...
    Node* ptr = start;
    while(ptr!=NULL) {
        printf("\t\tst>>> Passenger Name:    ");
        puts(ptr->name);

        printf("\t\tst>>>Passenger Gender:   ");
        puts(ptr->gen);
...
void cancel_position(Node **start,int j)    //funtion use for cancel the passenger booking                 //j=no. of passengers,y=bill amount
{
    int i,pos;
    char nameg[30];
   ...
   Node* previous=*start;
printf("\n\n\n");
   printf("enter position you have to cancel ticket>>>>     ");
   scanf("%d",&pos);
...
void ticket( int j)                     //j=no. of passengers,y=bill amount
{
    int i;
    long int m;
    Node* ptr = start;
